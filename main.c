...
void DrwaGameframe()
{
	...
	for(i=1;i<Frame_height;i++)
	{
		...
		printf("┋");	
		a[FrameX+2*Frame_width-2][FrameY+i]=2;//标记右横框为游戏边框，防止方块越界 
	} 
}
...
{
	int k,K,del_rows=0;//分别用于记录某行方块的个数和删除方块的行数的变量 
...
void Gameplay()
{
	...
	tetris->level=1;//初始为第一关 
	while(1)//循环产生方块，直至游戏结束 
	{
		Flag(tetris);//得到产生俄罗斯方块类型的序号 
		Temp=tetris->flag;//记住当前俄罗斯方块的序号 
		tetris->x=FrameX+2*Frame_width+6;//获得预览界面方块X的坐标 
		tetris->y=FrameY+10;//获得预览界面方块Y的坐标
		tetris->flag = tetris->next;//获得下一个俄罗斯方块的序号 
		PrintTetris(tetris);//调用打印俄罗斯方块的方法 
		
		tetris->x=FrameX+Frame_width;//获得游戏窗口中心方块的X坐标 
		tetris->y=FrameY-1;//获得游戏窗口中心方块的Y坐标 
		tetris->flag=Temp;
		...
		while(1)
		{
			...
			if(kbhit())//判断键盘 
			{
				...
				if(ch==72)//上键 
				{
					if( tetris->flag >= 2 && tetris->flag <= 3)//一字 
					{
						tetris->flag++;
						tetris->flag%=2;
						tetris->flag+=2;
					}
					if(tetris->flag>=4 && tetris->flag<=7)//T字 
					{
						tetris->flag++;
						tetris->flag%=4;
						tetris->flag+=4;
					}
					if(tetris->flag>=8 && tetris->flag<=11)//Z字 
					{
						tetris->flag++;
						tetris->flag%=4;
						tetris->flag+=8;
					}
					if(tetris->flag>=12 && tetris->flag<=15)//7字 
					{
						tetris->flag++;
						tetris->flag%=4;
						tetris->flag+=12;
					}
					if(tetris->flag>=16 && tetris->flag<=19)//反7字 
					{
						tetris->flag++;
						tetris->flag%=4;
						tetris->flag+=16;
		...
		tetris->flag = tetris->next;//清除右边窗口的图形 
